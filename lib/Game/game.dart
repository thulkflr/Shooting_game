...
import 'package:shooting_game/Game/player.dart';
import 'package:shooting_game/Game/bullet.dart';
...
class MyGame extends FlameGame with PanDetector, HasCollisionDetection {
  ...
  Future<void>? onLoad() async {
    final playerImage = await images.load('spaceship.png');
    ...
    final opponentImage = await images.load('oponent_spaceship.png');
    _opponent = Player(isMe: false);
    _opponent.add(SpriteComponent.fromImage(opponentImage, size: spriteSize));
    add(_opponent);

    _playerBulletImage = await images.load('bullet.png');
    _opponentBulletImage = await images.load('oponentbullet.png');
